...
def parse_example_proto(example_serialized, has_3d=False):
    ...
    feature_map = {
        ...
        tf.io.FixedLenFeature([], dtype=tf.string, default_value=''),
        'image/center':
        tf.io.FixedLenFeature((2, 1), dtype=tf.int64),
        'image/visibility':
        tf.io.FixedLenFeature((1, 14), dtype=tf.int64),
        'image/x':
        tf.io.FixedLenFeature((1, 14), dtype=tf.float32),
        'image/y':
        tf.io.FixedLenFeature((1, 14), dtype=tf.float32),
        'image/face_pts':
    ...
    fname = tf.compat.v1.Print(fname, [fname], message="image name: ")
...
def read_smpl_data(filename_queue):
    ...
    It's contents are:
        ...
        'shape' : 10-D float
    """
    with tf.name_scope(name='read_smpl_data'):
...
def decode_jpeg(image_buffer, name=None):
    ...
    """
    with tf.compat.v1.name_scope(name, 'decode_jpeg', [image_buffer]):
...
def jitter_center(center, trans_max):
    with tf.name_scope(name='jitter_center'):
...
def jitter_scale(image, image_size, keypoints, center, scale_range):
    with tf.name_scope(name='jitter_scale'):
        scale_factor = tf.random.uniform(
            [1],
            minval=scale_range[0],
            maxval=scale_range[1],
            dtype=tf.float32)
        new_size = tf.cast(tf.cast(image_size, dtype=tf.float32) * scale_factor, dtype=tf.int32)
        new_image = tf.image.resize(image, new_size)

        # This is [height, width] -> [y, x] -> [col, row]
        actual_factor = tf.cast(
            tf.shape(input=new_image)[:2], dtype=tf.float32) / tf.cast(image_size, dtype=tf.float32)
...
def pad_image_edge(image, margin):
    ...
    def repeat_col(col, num_repeat):
        ...
        with tf.name_scope(name='repeat_col'):
            return tf.reshape(
                tf.tile(tf.reshape(col, [-1]), [num_repeat]),
                [num_repeat, -1, 3])

    with tf.name_scope(name='pad_image_edge'):
        ...
        left = tf.transpose(a=repeat_col(image[:, 0, :], margin), perm=[1, 0, 2])
        right = tf.transpose(
            a=repeat_col(image[:, -1, :], margin), perm=[1, 0, 2])
...
def random_flip(image, kp, pose=None, gt3d=None):
    ...
    if pose is not None:
        new_image, new_kp, new_pose, new_gt3d = tf.cond(
            pred=mirror_cond, true_fn=lambda: flip_image(image, kp, pose, gt3d),
            false_fn=lambda: (image, kp, pose, gt3d))
        return new_image, new_kp, new_pose, new_gt3d
    else:
        new_image, new_kp = tf.cond(pred=mirror_cond, true_fn=lambda: flip_image(image, kp),
                                    false_fn=lambda: (image, kp))
...
def flip_image(image, kp, pose=None, gt3d=None):
    ...
    new_x = tf.cast(tf.shape(input=image)[0], dtype=kp.dtype) - kp[0, :] - 1
    ...
    new_kp = tf.transpose(a=tf.gather(tf.transpose(a=new_kp), swap_inds))
...
def reflect_pose(pose):
    ...
    """
    with tf.compat.v1.name_scope("reflect_pose", [pose]):
        """
...
def reflect_joints3d(joints):
    ...
    with tf.compat.v1.name_scope("reflect_joints3d", [joints]):
        joints_ref = tf.gather(joints, swap_inds)
        flip_mat = tf.constant([[-1, 0, 0], [0, 1, 0], [0, 0, 1]], tf.float32)
        joints_ref = tf.transpose(
            a=tf.matmul(flip_mat, joints_ref, transpose_b=True))
        # Assumes all joints3d are mean subtracted
        joints_ref = joints_ref - tf.reduce_mean(input_tensor=joints_ref, axis=0)
        return joints_ref
