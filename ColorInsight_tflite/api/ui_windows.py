...
def get_height_from_api(image_path):
    url = "http://127.0.0.1:10501/predict/height"     # your FastAPI URL
...
def analizar_imagen():
    ...
    try:
        ...
        height_cm = get_height_from_api("captured_image.jpg")
       

        if height_cm != None:
            to_cut = cv2.imread("captured_image.jpg")
            cut_image = to_cut[30:150,260:380]
            cv2.imwrite("face_captured.jpg", cut_image)
            ...
            altura_calculada.config(text=height_cm)
            result_text.place(x=10, y=400)
            result_text.config(text=full_text)
        else:
            altura_calculada.config(text="No se detectó a una persona")
...
altura_calculada = Label(app, text=" ", font=normal_font, fg="black", bg="white")
