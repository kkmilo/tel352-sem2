...
def keypoint_l1_loss(kp_gt, kp_pred, scale=1., name=None):
    ...
    Inputs:
      ...
      kp_pred: N x K x 2
    """
    with tf.compat.v1.name_scope(name, "keypoint_l1_loss", [kp_gt, kp_pred]):
...
def compute_3d_loss(params_pred, params_gt, has_gt3d):
    ...
    """
    with tf.compat.v1.name_scope("3d_loss", [params_pred, params_gt, has_gt3d]):
...
def align_by_pelvis(joints):
    ...
    """
    with tf.compat.v1.name_scope("align_by_pelvis", [joints]):
        left_id = 3
        right_id = 2
