...
import sistema_altura.scripts.predicciones_headless
...
def analizar_imagen():
    ...
    try:
        ...
        analisis.place(x=950, y=750)
        height_label.place(x=350, y=750)

        app.update_idletasks()  # show "Analizando imagen..." before processing

        #aqui hay que correr MODELO ALTURA

        # Get height value
        height_cm = sistema_altura.scripts.predicciones_headless.main()

        altura_calculada = Label(app, text=height_cm, font=normal_font, fg="black", bg="white")
        altura_calculada.place(x=550, y =750)
    ...
    button_finish.place(x=250, y=750)
...
def mostrar_ocultar_bienvenida():
    ...
    if button_iniciar.winfo_ismapped():  # if INICIAR is visible → go to camera
        button_iniciar.place_forget()
        subtitle.place_forget()

        video_label.place(x=900, y=200)
        button_foto.place(x=1000, y=800)
        ...
        for widget in [video_label, analisis, result_text, button_foto, button_finish,
                       height_label, prenda_label, prenda_menu]:
            widget.place_forget()
        titulo.place(x=50, y=30)
        subtitle.place(x=50, y=700)
        button_iniciar.place(x=250, y=450)
...
# === Buttons ===
button_iniciar = Button(app, text="INICIAR", font=("Times New Roman", 50, "bold"),
                 fg="white", bg="#b75e66", command=lambda: mostrar_ocultar_bienvenida())
button_foto = Button(app, text="Tomar foto", font=("Times New Roman", 50, "bold"),
                 fg="white", bg="#b75e66", command=analizar_imagen)
button_finish = Button(app, text="Finalizar", font=("Times New Roman", 50, "bold"),
...
def resize_ui(event=None):
    ...
    if button_iniciar.winfo_ismapped():
        button_iniciar.place_configure(x=w * 0.25, y=h * 0.45)
    if video_label.winfo_ismapped():
        video_label.place_configure(x=w * 0.55, y=h * 0.15)
    if button_foto.winfo_ismapped():
        button_foto.place_configure(x=w * 0.60, y=h * 0.75)
    if button_finish.winfo_ismapped():
        button_finish.place_configure(x=w * 0.15, y=h * 0.80)
...
def mostrar_resultado(resultado_texto):
    ...
    button_finish.place(x=250, y=850)
